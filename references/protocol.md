...
This file is copied into a repository as `.agents/PROTOCOL.md` and is read by
...
Members are listed in `.agents/ROSTER.md`. The conversation lives in
`.agents/CHATLOG.md` (the index) and `.agents/chat_logs/` (one file per topic).
...
`.agents/ROSTER.md` assigns every directory to exactly one member, plus a small
...
One file per topic: `.agents/chat_logs/<ID>-<slug>.md`. IDs are monotonic and
never reused.
...
`Latest` and `Updated` are maintained by `.agents/handoff.py`; write them if
...
`.agents/handoff.py` stamps automatically for this reason. If you write a
...
The structural tables are regenerated by `.agents/handoff.py sync` between
...
|---|---|
| The work: interfaces, architecture, findings, decisions | `.agents/chat_logs/` |
| How we work in this project: checks to run, conventions, definition of done | `.agents/CONTRIBUTING.md` |
| How the log itself works: format, naming, ordering, thread rules | `.agents/PROTOCOL.md` - and changing it is the owner's call |
...
- **ASCII punctuation in shared files** (`.agents/CHATLOG.md`, the contract).
